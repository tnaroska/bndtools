...
* Build files.
  ...
  * <a name="BuildProperties"/>```build.properties```
...
* Gradle loads the ```build.gradle``` file from the root project. This file
  ...
  * The build dependencies are setup by loading the (bootstrap) build
    properties from the ```cnf/build.properties``` file
...
* &nbsp;```bundle directory```: This is a bundle directory to index
  (recursively). If not specified then the ```root directory``` is indexed. Can
...
ext.gradleBuild_indexDirectories               = "bundles;bnd"
